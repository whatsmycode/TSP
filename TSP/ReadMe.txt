================================================================================
    MICROSOFT 基础类库 : TSP 项目概述
===============================================================================

应用程序向导已为您创建了此 TSP 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。
...
TSP.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CTSPApp 应用程序类。

TSP.cpp
    这是包含应用程序类 CTSPApp 的主应用程序源文件。

TSP.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\TSP.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 TSP.rc 中。
...
res\Toolbar.bmp
    此位图文件用于为工具栏创建平铺图像。
    初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 TSP.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////

应用程序向导创建一种文档类型和一个视图：

TSPDoc.h、TSPDoc.cpp - 文档
    这些文件包含 CTSPDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 CTSPDoc::Serialize）。
    该文档将包含以下字符串：
        文件扩展名：      tsp
        文件类型 ID：        TSP.Document
        主框架标题：  TSP
        文档类型名称：       TSP
        筛选器名称：         TSP Files (*.tsp)
        文件的新短名称：TSP
        文件类型长名称：TSP.Document

TSPView.h、TSPView.cpp - 文档视图
    这些文件包含 CTSPView 类。
    CTSPView 对象用于查看 CTSPDoc 对象。
